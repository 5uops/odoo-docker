...
$.summernote.lang.odoo = {
    ...
    image: {
      image: _t('图片'),
      insert: _t('插入图片'),
      resizeFull: _t('重置为100%'),
      resizeHalf: _t('重设为50%'),
      resizeQuarter: _t('重设为方形'),
      floatLeft: _t('左侧浮动'),
      floatRight: _t('右侧浮动'),
      floatNone: _t('不浮动'),
      dragImageHere: _t('拖动一个图片到这里'),
      selectFromFiles: _t('从文件中选择'),
      url: _t('图片网址'),
      remove: _t('移除图片')
    },
    link: {
      ...
      unlink: _t('取消链接'),
      edit: _t('编辑'),
      textToDisplay: _t('要显示的文本'),
      url: _t('这个链接指向的网址?'),
      openInNewWindow: _t('在新窗体中打开')
    },
    video: {
      video: _t('视频'),
      videoLink: _t('视频链接'),
    ...
    color: {
      ...
      foreground: _t('前景颜色'),
      transparent: _t('透明'),
      setTransparent: _t('设置为透明'),
      reset: _t('Reset'),
      resetToDefault: _t('重设为默认值')
    },
    shortcut: {
      shortcuts: _t('键盘快捷键'),
      close: _t('关闭'),
